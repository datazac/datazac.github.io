...
Physics(function(world){
  ...
  //hover color play pause
  // $('.start-stop').hover(function(){
  ...
  // dot radius
  if ($(window).width() > 960) { // large screen
    var radius = 60;
  } else if ($(window).width() <= 960 && $(window).width() > 480) { // retina landscape
    var radius = 45;
  ...
  function onElementMouseDown( event ) {
    ...
    if ( event.target == document.getElementById( 'jobbook' ) ) {
        ...
        var textLeft = "My  first experience working in the web. I was lucky enough to get an opportunity at this start-up on my return from Rio de Janeiro. I began as an intern and when I left a year and half later, I was lead project manager. I was the first non-technical full time employee. I worked side by side everyday with the founders. Jobbook still attempts to change the face of online recruiting, and while I was there we were totally bootstrapped.";
...
//doughnut charts
    ...
    // change chart size based on screen width
    // if ($(window).width() > 480) {
    //   chartWidth = 60;
    //   chartHeight = 60;
    // } else {
